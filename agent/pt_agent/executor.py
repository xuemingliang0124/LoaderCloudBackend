...
TODO P1 剩余：
1. _metrics_loop   JTL 增量解析，产出真实指标
2. Master 侧结果分片按 label 合并（orchestrator.on_agent_result）
"""
...
class TaskExecutor:
    ...
    async def submit(self, data: dict) -> None:
        ...
        if not run_id:
            return
        if self.busy:
            await self._reporter.send_task_ack(run_id, accepted=False, message="Agent 忙碌中")
            return
        self._stopping = False
    ...
    async def stop(self, run_id: str) -> None:
        ...
        if self._state.current_run_id != run_id:
            await self._reporter.send_task_ack(run_id, accepted=False, message="未在执行该任务")
            return
        self._stopping = True
    ...
    async def _execute(self, data: dict) -> None:
        ...
        jmeter_args = {str(k): str(v) for k, v in (data.get("jmeter_args") or {}).items()}
        try:
            ...
            jmx_path = await self._prepare_files(run_id, data.get("files") or [], run_dir)
            ...
            await self._runner.start(run_id, jmx_path, jmeter_args, jtl_path, report_dir)

            metrics_task = asyncio.create_task(self._metrics_loop(run_id))
            ...
            if exit_code != 0 and not self._stopping:
                raise RuntimeError(f"JMeter 执行失败，退出码 {exit_code}，请查看 JMeter 控制日志")
            ...
            # TODO P1: 解析 JTL 生成真实汇总（samples/errors/p95_rt/max_tps）
            summary = {
                "failed": self._stopping,
                "exit_code": exit_code,
                "samples": 0,
                "errors": 0,
                "p95_rt": 0.0,
            ...
            await self._reporter.send_status(run_id, AgentPhase.FAILED, message=str(exc))
            await self._reporter.send(
                Envelope.now(
                    MSG_RESULT,
                    {"run_id": run_id, "summary": {"failed": True, "message": str(exc)}, "artifacts": []},
                )
            )
        finally:
    ...
    async def _metrics_loop(self, run_id: str) -> None:
        ...
        TODO P1: tail 解析 JTL 增量，按 label 计算 TPS/RT 分位数/错误率。
        骨架阶段上报占位零值，保证链路贯通。
        """
        settings = get_settings()
        while True:
    ...
    async def _upload_artifacts(
        ...
        if report_target.get("url") and Path(report_dir).is_dir():
            ...
            try:
                ...
                artifacts.append({"type": "report", "key": report_target.get("key", "")})
                logger.info(f"[{run_id}] HTML 报告已上传: {report_target.get('key')}")
            except Exception as exc:  # noqa: BLE001
                logger.warning(f"[{run_id}] 报告上传失败（不影响结果上报）: {exc}")
        else:
            logger.warning(f"[{run_id}] 报告目录不存在或缺少上传地址，跳过: {report_dir}")
        return artifacts
    ...
    async def _upload_file(url: str, path: str) -> None:
        ...
        def _put() -> None:
            with open(path, "rb") as f, httpx.Client(timeout=httpx.Timeout(600.0)) as client:
