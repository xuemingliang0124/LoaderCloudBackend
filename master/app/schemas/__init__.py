...
class TestPlanScenarioIn(BaseModel):
    """方案挂载场景项：seq 决定批量执行顺序，weight 预留报告聚合语义。"""

    scenario_id: int = Field(..., examples=[1])
    seq: int = Field(default=0, ge=0, examples=[1])
    weight: int = Field(default=1, ge=1, examples=[1])


class TestPlanScenarioOut(BaseModel):
    # scenario_name 非 ORM 列：由接口层按 scenario_id 关联 test_scenario.name 填充
    scenario_id: int
    scenario_name: str = ""
    seq: int
    weight: int
...
class TestPlanIn(BaseModel):
    """新建测试方案请求：项目内 name 唯一；scenarios 创建时一并挂载。"""
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "description": "登录+下单+支付全链路回归",
                "scenarios": [
                    {"scenario_id": 1, "seq": 1, "weight": 1},
                    {"scenario_id": 2, "seq": 2, "weight": 2},
                ],
            }
        }
    )

    name: str = Field(..., min_length=1, max_length=128, examples=["核心交易回归方案"])
    # 整体通过判据（自由 JSON，结构由报告/LLM 模块解释）
    pass_criteria: dict = Field(default_factory=dict)
    report_template: str = Field(default="default", max_length=64)
    description: str = Field(default="", max_length=512)
    scenarios: list[TestPlanScenarioIn] = Field(default_factory=list)

    @field_validator("name")
    @classmethod
    def _strip_and_require(cls, v: str) -> str:
        # 与环境/交易口径一致：去除首尾空白，纯空白视为非法（422）
        v = v.strip()
        if not v:
...
class TestPlanUpdateIn(BaseModel):
    """更新测试方案请求：所有字段可选，至少传一项；scenarios 传则全量替换挂载。"""

    name: str | None = Field(default=None, min_length=1, max_length=128)
    pass_criteria: dict | None = None
    report_template: str | None = Field(default=None, max_length=64)
    description: str | None = Field(default=None, max_length=512)
    scenarios: list[TestPlanScenarioIn] | None = None
    ...
    @model_validator(mode="after")
    def _at_least_one(self) -> "TestPlanUpdateIn":
        fields = (
            self.name,
            self.pass_criteria,
...
class TestPlanOut(BaseModel):
    ...
    created_at: datetime
    updated_at: datetime
    # 由接口层装配（ORM 关系 plan_scenarios + scenario 名称），非 from_attributes 直出
    scenarios: list[TestPlanScenarioOut] = []
