...
- 删除遵循「预检 + force」模式：
  A3 场景绑定 environment_id 后，被场景引用的环境严格模式拒绝（3043），
  force=true 先批量解绑引用场景（environment_id 置 null）再删环境。
"""
...
async def precheck_environment_delete(
    ...
    统计 scenario.environment_id == env_id 的场景数；引用数 > 0 时严格模式
    拒绝删除（3043），需先解绑场景或携带 force=true 强制解绑再删。
    """
    await ensure_project_access(db, project_id, user, "viewer")
    ...
    scenarios = (
        await db.scalar(
            ...
            .where(Scenario.environment_id == env_id)
        )
    ) or 0
    return ok({"environment_id": env_id, "scenarios": int(scenarios)})
...
async def delete_environment(
    ...
    force: bool = Query(
        False,
        description="强制删除：先把引用该环境的场景解绑（environment_id 置 null）再删",
    ),
    db: AsyncSession = Depends(get_db),
    user: CurrentUser = Depends(get_current_user),
) -> dict:
    ...
    严格模式：被场景引用时拒绝（3043），需先解绑场景或携带 force=true；
    force 模式：先批量解绑引用场景（scenario.environment_id=NULL）再删环境，
    DB 层 ondelete=SET NULL 作为兜底。
    """
    ...
    if not force and referencing_scenarios:
        raise BusinessError(
            f"环境被 {int(referencing_scenarios)} 个场景引用，无法删除；"
            "请先解绑场景或携带 force=true 强制删除",
            code=3043,
        )

    if force and referencing_scenarios:
        # 解绑引用该环境的场景（environment_id 置 null，不删场景）
        await db.execute(
            sa_update(Scenario)
            .where(Scenario.environment_id == env_id)
    ...
    return ok(
        {
            ...
            "deleted": True,
            "force": force,
            "removed_scenarios": int(referencing_scenarios) if force else 0,
        }
    )
