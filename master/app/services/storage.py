...
async def get_object_bytes(object_key: str) -> bytes:
    """读取 MinIO 对象为 bytes（用于插件 sha256 / JMX 扫描等小文件场景）。

    miniopy_async.get_object 返回 aiohttp.ClientResponse，用 read() 一次性
    读取 body（脚本/JMX 等均小文件）；response 需显式 release 释放连接。
    """
